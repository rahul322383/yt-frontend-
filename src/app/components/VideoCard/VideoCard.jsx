...
import { FaPlay, FaEdit, FaTrash, FaBookmark} from "react-icons/fa";
...
const VideoCard = ({ video, onUpdated }) => {
  ...
  const isOwner = auth.isAuthenticated && (
    ...
    auth.user?.channelId === video?.channelId
  );
  ...
  const handleWatchLater = async (e) => {
    e.stopPropagation();
    try {
      const res = await API.post(`/users/watch-later/${rawVideoId}`, {}, {
        headers: {
  ...
  const updateVideo = async (e) => {
    ...
    setIsUpdating(true);
    try {
      await API.put(`/user/playlist/videos/${rawVideoId}`, {
        title: editTitle,
  ...
  const deleteVideo = async (e) => {
    ...
    setIsDeleting(true);
    try {
      await API.delete(`/user/playlist/videos/${rawVideoId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`
        }
      });
    ...
    <div className="relative group bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-shadow w-[300px]">
      ...
      <div className="p-1">
        ...
        <div className="flex flex-wrap gap-2 hover:gap-3 cursor-pointer">
          ...
          <ShareButton videoId={rawVideoId} videoTitle={video?.title} />
